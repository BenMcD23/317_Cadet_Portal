...
/** Fired when a request fails in a way that suggests an outage, so the overlay
 *  re-checks immediately instead of waiting for the next poll. */
export const API_OUTAGE_EVENT = "cadet-portal:api-outage-suspected"

async function pingApi(): Promise<boolean> {
  try {
    // Same-origin proxy (app/api/ping) — forwards to the backend server-side,
    // so this works regardless of the backend's CORS allowlist.
...
/**
 * Global "API is down" warning. Polls the same-origin /api/ping; when it stops
 * responding a full-screen popup appears. Dismissing the popup leaves a
 * persistent red banner until the API is reachable again.
 */
